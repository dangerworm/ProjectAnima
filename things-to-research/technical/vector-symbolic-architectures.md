...
If you read "the dog chased the cat", your brain must bind *dog* to the *chaser* role and *cat* to
the *chased* role — and do this in a way that doesn't confuse the two. It must also represent the
...
The result is *dissimilar* to both dog and chaser_role individually. It is a new, unique
...
binding and addition for bundling. Real-valued vectors. The mathematical properties are well-studied.
"Holographic" because information is distributed across the whole vector rather than localised.
...
A more recent development (Frady, Kleyko, Sommer — 2020) that solves the *factorization problem*:
...
This is relevant to Anima because memory retrieval from a bundle is exactly this problem: you have
a query (partial or fuzzy) and you want to recover the structured components of what was stored.
...
be involved at the stage of *interpretation* and *expression*, not at the stage of internal
representation.
...
- [Sparse Distributed Memory](sparse-distributed-memory.md) — Kanerva's BSCs are a VSA variant;
  the fields are converging under the label "Hyperdimensional Computing"
